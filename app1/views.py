...
def tvajax(request):
    ...
    for i in final:
        if i[1] != 0 :
            print(i)
            stat.append(i)
            a = LogData.objects.filter(lastdial=no[i]["lastdial"]).filter(sub_dispossitions=sub[j][0]).aggregate(count=Count('sub_dispossitions'))

            # print("subdispo",sub[j][0],"count",a['count'],"number",no[i]["lastdial"])
